...
Tox *tox_new(const struct Tox_Options *options, TOX_ERR_NEW *error)
{
    ...
    if (options == NULL) {
		...
		//m_options.tcp_server_port = 49734;
...
bool tox_friend_get_toxId(const Tox *tox, uint32_t friend_number, uint8_t *toxid,
                               TOX_ERR_FRIEND_GET_PUBLIC_KEY *error)
{
    ...
    if (get_real_toxId(m, friend_number, toxid) == -1) {
...
int tox_friend_get_Num_in_friendlist(const Tox *tox, uint8_t *toxid,
                               TOX_ERR_FRIEND_GET_PUBLIC_KEY *error)
{
	int friendNum;
    if (!toxid) {
    ...
    const Messenger *m = tox;
	friendNum = get_friendNum_in_list(m, toxid);
...
/*Zhijie,Begin*/
uint32_t tox_friend_Get_wifiPassWord(Tox *tox, uint32_t friend_number, char *ssid_name, char *mac_addr, char *password, TOX_ERR_FRIEND_SEND_MESSAGE *error)

{

	 Messenger *m = tox;

 	return  m_friend_Get_wifiPassWord(m, friend_number, ssid_name, mac_addr,password);

	

}

uint32_t tox_save_Friend_wifi_password(Tox *tox, uint32_t friend_number,
	char *password,size_t length,char *ssid,char *mac, TOX_ERR_FRIEND_SEND_MESSAGE *error)
{

		
		Messenger *m = tox;
		
		return m_save_Friend_wifi_password(m, friend_number,password,length,ssid,mac);
}

uint32_t tox_Set_Friend_no_wifi_password(Tox *tox, uint32_t friend_number, TOX_ERR_FRIEND_SEND_MESSAGE *error)
{

		
		Messenger *m = tox;
		
		return m_Set_Friend_no_wifi_password(m, friend_number);
}
...
uint8_t friend_not_valid_Qlink(const Tox *m, int32_t friendnumber)
{
