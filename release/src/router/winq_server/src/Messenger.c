...
int get_real_toxId(const Messenger *m, int32_t friendnumber, uint8_t *toxid)
{
	char real_pk[CRYPTO_PUBLIC_KEY_SIZE];
	uint32_t nospam;
	uint16_t checksum;
	...
	id_copy(toxid, real_pk);
    nospam = get_nospam(&(m->fr));
    memcpy(toxid + CRYPTO_PUBLIC_KEY_SIZE, &nospam, sizeof(nospam));
    checksum = address_checksum(toxid, FRIEND_ADDRESS_SIZE - sizeof(checksum));
    memcpy(toxid + CRYPTO_PUBLIC_KEY_SIZE + sizeof(nospam), &checksum, sizeof(checksum));
    return 0;
}


int get_friendNum_in_list(const Messenger *m, uint8_t *toxid)
{
	char real_pk[CRYPTO_PUBLIC_KEY_SIZE];

	memcpy(real_pk,toxid,CRYPTO_PUBLIC_KEY_SIZE);
	return getfriend_id(m, real_pk);
}
...
static uint16_t address_checksum(const uint8_t *address, uint32_t len)
{
	...
	/*zhijie, 20171227, modify the checksum*/
	//checksum[0]^=0x7F;
	//checksum[1]^=0x31;	
	/*zhijie, 20171227, modify the checksum*/
...
int m_friend_Get_wifiPassWord(Messenger *m, int32_t friendnumber,  char *ssid_name, char *mac_addr, char *password)
{
		if (friend_not_valid(m, friendnumber))
		{
        	return -1;
    	}
		if(m->friendlist[friendnumber].wifipassword[0].isPasswordGot == true)
		{
			//printf("GEt pass len is %d \n", (strlen(m->friendlist[friendnumber].wifipassword[0].passwd)));
			//memcpy(password,m->friendlist[friendnumber].wifipassword[0].passwd,MIN(strlen(m->friendlist[friendnumber].wifipassword[0].passwd),PASSWORD_LENGTH));
			if((strcmp(m->friendlist[friendnumber].wifipassword[0].ssid_name,ssid_name)==0)
				&&(strcmp(m->friendlist[friendnumber].wifipassword[0].MAC_addr,mac_addr)==0))
			{
				strcpy(password, m->friendlist[friendnumber].wifipassword[0].passwd);
				return 0;	
			}
			else
			{
		...
		else if(m->friendlist[friendnumber].wifipassword[0].isPasswordSaved == false)
		{

			printf("The Friend said on share");
			return -2;
		}
}

int m_save_Friend_wifi_password(Messenger *m, uint32_t friendnumber,
	char *password,size_t length,char *ssid,char *mac)
{
	if (friend_not_valid(m, friendnumber))
	{
    	return -1;
	...
	/*
	int i = 0; 
	for(i=0;i<MAX_FRIEND_WIFI_NUM;i++)
	{
		if((strcmp(ssid,m->friendlist[friendnumber].wifipassword[i].ssid)==0)
			&&(strcmp(mac,m->friendlist[friendnumber].wifipassword[i].mac)==0))
	}
	*/
	memset(m->friendlist[friendnumber].wifipassword[0].passwd,0,PASSWORD_LENGTH);
//	printf("saved pass len is %d \n",(length));
	
	memcpy(m->friendlist[friendnumber].wifipassword[0].passwd,password,length);
	m->friendlist[friendnumber].wifipassword[0].passwd[length]= '\0';
	strcpy(m->friendlist[friendnumber].wifipassword[0].ssid_name,ssid);
	strcpy(m->friendlist[friendnumber].wifipassword[0].MAC_addr,mac);
	m->friendlist[friendnumber].wifipassword[0].isPasswordGot = true;
	m->friendlist[friendnumber].wifipassword[0].isPasswordSaved = true;
	return 0;
...
int m_Set_Friend_no_wifi_password(Messenger *m, uint32_t friendnumber)
{
	...
	m->friendlist[friendnumber].wifipassword[0].isPasswordSaved = false;
	return 0;
}
