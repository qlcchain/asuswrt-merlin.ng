...
#else
    if (sodium_init() == -1) {
        DEBUG_PRINT(DEBUG_LEVEL_ERROR,"networking_at_startup:sodium_init failed");
        return -1;
    }
...
Networking_Core *new_networking_ex(Logger *log, IP ip, uint16_t port_from, uint16_t port_to, unsigned int *error)
{
    ...
    if (ip.family != TOX_AF_INET && ip.family != TOX_AF_INET6) {
        ...
        DEBUG_PRINT(DEBUG_LEVEL_ERROR,"new_networking_ex:ip.family (%d)",ip.family );
        return NULL;
    }
    if (networking_at_startup() != 0) {
        DEBUG_PRINT(DEBUG_LEVEL_ERROR,"new_networking_ex:networking_at_startup failed");
        return NULL;
    }
    ...
    if (!sock_valid(temp->sock)) {
        ...
        DEBUG_PRINT(DEBUG_LEVEL_ERROR,"sock_valid valid");
        free(temp);
        if (error) {
            *error = 1;
        }
     ...
     */
#if 0 //changed by willcao,openwrt中socket缓存区没那么大，使用默认大小
    int n = 1024 * 1024 * 2;
    ...
    if (!set_socket_nosigpipe(temp->sock)) {
        ...
        DEBUG_PRINT(DEBUG_LEVEL_ERROR,"new_networking_ex:set_socket_nosigpipe failed");
        return NULL;
    }

    /* Set socket nonblocking. */
    if (!set_socket_nonblock(temp->sock)) {
        DEBUG_PRINT(DEBUG_LEVEL_ERROR,"new_networking_ex:set_socket_nonblock failed");
        kill_networking(temp);
